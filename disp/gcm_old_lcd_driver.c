...
void init_LCD(void)
{
	...
	LCD_DDR |= (1<<LCD_DC) | (1<<LCD_RESET) | (1<<LCD_SELECT) | (1<<LCD_CLK) | (1<<LCD_MOSI); // sets the pins for the LCD as outputs
...
void LCD_Write(uint8_t dc, uint8_t data)
{
	...
	LCD_PORT &= ~(1<<LCD_SELECT); // enables the receiving of data or commands
	
	LCD_ShiftOut(LCD_MOSI, LCD_CLK, data); 
	
	LCD_PORT |= (1<<LCD_SELECT); // disables the receiving of data or commands 
}
